...
class BannedCustomerPage {
  constructor(page) {
    ...
    this.customerNameInput =
      '(//label[text()="Customer Name"] |  //input[@tvalue="string"])[2]';
    this.idProofDocumentInput = ".rz-dropdown";
    this.idProofNoInput =
      '(//label[text()="ID Proof Number"] |  //input[@tvalue="string"])[3]';
    this.submitButton = "#submitButton";
    this.custmoreNameWarning = '//p[text()="Please enter the Customer Name."]';
    this.selectIDProofDocumentWarning =
      '//p[text()="Please Select ID Proof Document."]';
    ...
    this.idProofNumberWarning =
      '//p[text()="Please enter Valid ID Proof Number."]';
    ...
    this.mobileNumberInput =
      '(//label[text()="Mobile Number"]|//input[@type="text"])[2]';
    this.customerIdInput =
      '(//label[text()="Customer ID"]|//input[@tvalue="string"])[2]';
    ...
    this.addBannedPageAddressWarning =
      '//p[text()="Please enter the Address."]';
    ...
    this.addBannedPageAddressWarningSpecialchar =
      '//div[text()="Invalid Address"]';
  ...
  async verifyWarningMessageCustmoreName() {
    await this.page.waitForSelector(this.custmoreNameWarning, {
      state: "visible",
    });
  ...
  async enterCustomerName(ValidcustomerName) {
    await this.page.waitForSelector(this.customerNameInput, {
      state: "visible",
    });
    await this.page.fill(this.customerNameInput, ValidcustomerName);
    const custmoreNameValue = await this.page.inputValue(
      this.customerNameInput
    );
    const custmoreNameLength = custmoreNameValue.length;
    console.log(
      "Please Enter the character from the Custmore Name field should allowed 0-50 "
    );
    console.log(
      `Entered custmoreName: ${custmoreNameValue}, Entered custmoreName Length: ${custmoreNameLength}`
    );

    if (custmoreNameValue.length < 0) {
      throw new Error(
        "Customer name should not accept less than 0 characters."
      );
    } else if (custmoreNameValue.length > 50) {
      throw new error(
        "Customer name should not accept more than 50 characters."
      );
    }
  }
  async verifyWarningMessageselectIDProofDocument() {
    await this.page.waitForSelector(this.selectIDProofDocumentWarning, {
      state: "visible",
    });
    await expect(
      this.page.locator(this.selectIDProofDocumentWarning)
    ).toBeVisible();
  }
  ...
  async verifyWarningMessageIdProofNumber() {
    await this.page.waitForSelector(this.idProofNumberWarning, {
      state: "visible",
    });
    await expect(this.page.locator(this.idProofNumberWarning)).toBeVisible();
  }
  async enterIdProofNo(idNo) {
    await this.page.waitForSelector(this.idProofNoDropdown, {
      state: "visible",
    });
    ...
    console.log(
      "Please Enter the character from the ID Proof Number field should allowed is 0-20"
    );
    console.log(
      `Entered  ID Proof Number:${idProofNoValue},idProofNoLength:${idProofNoLength}`
    );
    if (idProofNoValue.length < 0) {
      throw new error(
        "ID Proof Number  should not accept less than 0 characters."
      );
    } else if (idProofNoValue.length > 20) {
      throw new error(
        "ID Proof Number  should not accept more than 20 characters."
      );
    }
  }
  ...
  async validationInputFieldsAfterReset() {
    ...
    const custmoreNameInput = await this.page
      .locator(this.customerNameInput)
      .inputValue();
    expect(custmoreNameInput).toBe(expectedcustmoreNameInput);
    console.log(
      "CustmoreName Input Field :" +
        expect(custmoreNameInput).toBe(expectedcustmoreNameInput)
    );

    await this.page.waitForSelector(this.selectDropdownText, {
      state: "visible",
    });
    const idProofDocumentInput = await this.page
      .locator(this.selectDropdownText)
      .textContent();
    expect(idProofDocumentInput).toContain(expectedidProofDocumentInput);
    console.log(
      "Id Proof Document Input Field:" +
        expect(idProofDocumentInput).toContain(expectedidProofDocumentInput)
    );

    await this.page.waitForSelector(this.idProofNoInput, { state: "visible" });
    const idProofNoInput = await this.page
      .locator(this.idProofNoInput)
      .inputValue();
    expect(idProofNoInput).toBe(expectedidProofNoInput);
    console.log(
      "Id Proof Number Input Field:" +
        expect(idProofNoInput).toBe(expectedidProofNoInput)
    );
  }
  async verifyWMessageCNameandIDPNumberSpecialCharacter() {
    await this.page.waitForSelector(this.alertMessageCustMoreNameSpecialch, {
      state: "visible",
    });
    expect(
      await this.page.locator(this.alertMessageCustMoreNameSpecialch)
    ).toBeVisible();
    await this.page.waitForSelector(this.alertMessageIDProofNumberSpecialch, {
      state: "visible",
    });
    expect(
      await this.page.locator(this.alertMessageIDProofNumberSpecialch)
    ).toBeVisible();
  }
  ...
  async enterCustomerID(customerIDInput) {
    ...
    const enteredCustomerIDValue = await this.page
      .locator(this.customerIdInput)
      .inputValue();
    console.log("CustmoreID input box Range should be  0-20");
    if (
      enteredCustomerIDValue.length >= 0 &&
      enteredCustomerIDValue.length <= 20
    ) {
      console.log(
        `CustmoreIDvalue:${enteredCustomerIDValue},CustmoreIDLength:${enteredCustomerIDValue.length}`
      );
    }
  }
  ...
  async enterMobileNumber(validMobileNumber) {
    await this.page.waitForSelector(this.mobileNumberInput, {
      state: "visible",
    });
    ...
    if (mobileNumberValue.length >= 0 && mobileNumberValue.length <= 10) {
      console.log(
        `Entered MobileNumber:${mobileNumberValue},Mobile Number Length:${mobileNumberValue.length}`
      );
    }
  }
  async addBandCustmoreCustmoreName(ValidcustomerName) {
    await this.page.waitForSelector(this.addBannedCustmoreCustmoreName, {
      state: "visible",
    });
    await this.page.fill(this.addBannedCustmoreCustmoreName, ValidcustomerName);
    const custmoreNameValue = await this.page.inputValue(
      this.addBannedCustmoreCustmoreName
    );
    const custmoreNameLength = custmoreNameValue.length;
    console.log(
      "Please Enter the character from the Custmore Name field should allowed 0-50 "
  ...
  async enterAddressInput(addressInput) {
    ...
    const inputAddressText = await this.page
    ...
    if (inputAddressText.length >= 0 && inputAddressText.length <= 100) {
      console.log(
        `Entered Address:${inputAddressText},Address Length:${inputAddressText.length}`
      );
    }
  }
  async addBandCustmoreIdProofNumber(validIDProofNumberAadhar) {
    await this.page.waitForSelector(this.addBannedIDProofNumberInput, {
      state: "visible",
    });
    await this.page.fill(
      this.addBannedIDProofNumberInput,
      validIDProofNumberAadhar
    );
    const idProofNumber = await this.page
      .locator(this.addBannedIDProofNumberInput)
      .inputValue();
    console.log("Id Proof Number text box should be allowed 0-12");
    if (idProofNumber.length >= 0 && idProofNumber.length <= 12) {
      console.log(
        `Entered Id Proof Number:${idProofNumber},IdProofNumber Length:${idProofNumber.length}`
      );
    }
  }
  async verifyAllInputValuesAddBanneCustmorePageAfterReset() {
    ...
    await this.page.waitForSelector(this.customerIdInput, { state: "visible" });
    const custmoreNameIDInput = await this.page
      .locator(this.customerIdInput)
      .inputValue();
    console.log(
      "Customer ID Input:" +
        expect(custmoreNameIDInput).toBe(expectedCustmoreId)
    );
    ...
    const custmoreNameInput = await this.page
      .locator(this.addBannedCustmoreCustmoreName)
      .inputValue();
    console.log(
      "Customer Name Input:" +
        expect(custmoreNameInput).toBe(expectedCustmoreName)
    );

    await this.page.waitForSelector(this.mobileNumberInput, {
      state: "visible",
    });
    const custmoreMobileNumberInput = await this.page
      .locator(this.mobileNumberInput)
      .inputValue();
    console.log(
      "Mobile Number Input:" +
        expect(custmoreMobileNumberInput).toBe(expectedMobileNumber)
    );
    ...
    console.log(
      "ID Proof Document Input:" +
        expect(idProofDocumentInput.trim()).toBe(expectedIDProofDocument.trim())
    );

    await this.page.waitForSelector(this.addBannedIDProofNumberInput, {
      state: "visible",
    });
    const idProofNumberInput = await this.page
      .locator(this.addBannedIDProofNumberInput)
      .inputValue();
    console.log(
      "ID Proof Number:" +
        expect(idProofNumberInput).toBe(expectedIDProofNumber)
    );
  }
  async addBannedCustmorePageCustmoreNameWarning() {
    const isVisible = await this.page
      .locator(this.addBannedCustmorePageCusmoreNameWarning)
      .isVisible();
    console.log(`Warning Message Custmore Name Is visible: ${isVisible}`);
    expect(isVisible).toBe(true);
  }
  async verifyWarningMessageMobilNumber() {
    await this.page.waitForSelector(this.addBannedPageMobileNumberWarning, {
      state: "visible",
    });
    const isVisible = await this.page
      .locator(this.addBannedPageMobileNumberWarning)
      .isVisible();
    console.log(`Warning Message Mobile Number Is visible: ${isVisible}`);
    expect(isVisible).toBe(true);
  }
  async verifyWarningMessageAddress() {
    await this.page.waitForSelector(this.addBannedPageAddressWarning, {
      state: "visible",
    });
    const isVisible = await this.page
      .locator(this.addBannedPageAddressWarning)
      .isVisible();
  ...
  async verifyWarningMessageIDProofNumber() {
    await this.page.waitForSelector(this.addBannedPageIDProofNumber, {
      state: "visible",
    });
    const isVisible = await this.page
      .locator(this.addBannedPageIDProofNumber)
      .isVisible();
    console.log(`Warning Message ID proof Number Is visible: ${isVisible}`);
    expect(isVisible).toBe(true);
  }
  async verifyWarningMessageCustomerID() {
    await this.page.waitForSelector(this.addBannedPageCustomerId, {
      state: "visible",
    });
    const isVisible = await this.page
      .locator(this.addBannedPageCustomerId)
      .isVisible();
  ...
  async verifyWarningMessageAddressSpecialChar() {
    ...
    const isVisible = await this.page
      .locator(this.addBannedPageAddressWarningSpecialchar)
      .isVisible();
    console.log(`Warning Message Address Is visible: ${isVisible}`);
    expect(isVisible).toBe(true);
  }
  ...
  async assertCheckboxCheckedStatus(expected) {
    const checkbox = await this.page.waitForSelector(this.statuscheckMark, {
      state: "visible",
    });
  ...
  async remarkFieldTextArea(expected) {
    ...
    const inputvalueRemark = await this.page
      .locator(this.remarkField)
      .inputValue();
    expect(inputvalueRemark).toBe(expected);
  }
  async verifyInputValuesFromCustomerDetailForm(
    cIDExpected,
    cNameExpected,
    addressExpected,
    idProofdocumentExpected,
    idProofNumberExpected,
    expectedMobileNumber
  ) {
    const customerIdActual = await this.page
      .locator(this.CustomerIDinput)
      .textContent();
    expect(customerIdActual).toBe(cIDExpected);

    const customerNameActual = await this.page
      .locator(this.customerNameInputCDetails)
      .textContent();
    expect(customerNameActual).toBe(cNameExpected);

    const addressActual = await this.page
      .locator(this.addressinput)
      .textContent();
    expect(addressActual).toBe(addressExpected);

    const idProofdocumentActual = await this.page
      .locator(this.idProofDocumentInput)
      .textContent();
    expect(idProofdocumentActual).toBe(idProofdocumentExpected);

    const idProofNumberActual = await this.page
      .locator(this.idProofNo)
      .textContent();
    expect(idProofNumberActual).toBe(idProofNumberExpected);

    const actualMobileNumber = await this.page
      .locator(this.mobileNoInput)
      .textContent();
  ...
  async confirmPopupCloseButton() {
    await this.page.waitForSelector(this.closeButtonPopup, {
      state: "visible",
    });
  ...
  async warningMessagesomeErrorOccured() {
    await this.page.waitForSelector(this.someErrorOccured, {
      state: "visible",
    });
    await expect(this.page.locator(this.someErrorOccured)).toBeVisible();
  }
  async SucessMessageForCustomerdetailsupdated() {
    await this.page.waitForSelector(this.customerdetailsupdated, {
      state: "visible",
    });
