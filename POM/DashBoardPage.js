...
class DasBoardPage {
  constructor(page) {
    ...
    //Usermanagement
    this.userManagement = `//a[@href="Admin/UserManagement"]`;
    this.amountInLakhs = `.rz-axis-title`;
    this.spinner = `//div[@class="spinner"]`;
  }
  async navigateToBannedCustomer() {
    await this.page.waitForSelector(
      this.securityManagementDropdown,
      { state: "visible" },
      { timeout: 60 * 1000 }
    );
    await this.page.locator(this.securityManagementDropdown).click();
    await this.page.waitForSelector(this.bannedCustomerOption, {
      state: "visible",
    });
  ...
  async navigateToAddNewCustomerPage() {
    await this.page.waitForSelector(this.addNewCustomerButton, {
      state: "visible",
    });
  ...
  async verifyIDProofDocumentTypesInSideDropdown() {
    await this.page.waitForSelector(this.idProofDocumentInput, {
      state: "visible",
    });
  ...
  async navigateToRoleManagement() {
    ...
    await this.page.waitForSelector(
      this.roleManagementOption,
      { state: "visible" },
      { timeout: 60 * 1000 }
    );
    await this.page.click(this.roleManagementOption, { timeout: 60 * 1000 });
  }

  async navigateToUserManagement() {
    await this.page.waitForSelector(this.securityManagementDropdown, {
      state: "visible",
    });
    await this.page.locator(this.securityManagementDropdown).click();
    await this.page.waitForSelector(this.userManagement, { state: "visible" });
